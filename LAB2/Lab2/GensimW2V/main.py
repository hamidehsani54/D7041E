...
while i < number_of_tests:
            ...
            try:
                ...
                vectors = []
                if words[0] in model.wv.key_to_index.keys(): # check if there embedding for the query word
                    k = 1 #counter for loop iterating over 5 words in the test
                    vectors.append(model.wv.__getitem__(words[0]))
                    while k < 5:
                        if words[k] in model.wv.key_to_index.keys(): # if alternative has the embedding
                            vectors.append(model.wv.__getitem__(words[k])) #assing the learned vector
                        else: 
